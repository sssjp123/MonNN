...
def create_model(
    ...
    k = len(monotonic_indices)
    lattice_sizes = [int(config["lattice_size"])] * k

    return HLLNetwork(
        dim=input_size,
        lattice_sizes=lattice_sizes,
        increasing=monotonic_indices,
        mlp_neurons=list(hidden_sizes),
        activation=nn.ReLU(),
...
def process_dataset(loader: Callable):
    ...
    if len(monotonic_indices) == 0:
        raise ValueError(
            f"HLL requires at least one monotonic feature, but got 0 for dataset: {loader.__name__}."
        )
