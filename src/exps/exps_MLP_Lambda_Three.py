...
def train_model(model, optimizer, train_loader, val_loader,
                config: Dict, task_type: str, device):
...
def optimize_hyperparameters(X, y, task_type):
    ...
    study.optimize(
        lambda trial: objective(trial, X, y, task_type),
        n_trials=20,
        n_jobs=1
    )
...
def main():
    ...
    for loader in dataset_loaders:
        print(f"\nProcessing dataset: {loader.__name__} (MLP Baseline)...")
        X, y = loader()
