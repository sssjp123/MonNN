...
def objective(trial, X_full: np.ndarray, y_full: np.ndarray, task_type: str, monotonic_indices: List[int],
    ...
    config = {
        ...
        "monotonicity_weight": current_lambda,  # ✅ Lambda fixed from external loop
...
def main():
    ...
    for idx, lambd in enumerate(lambda_list):
        ...
        csv_filenames = {
            "random": f"exps_MixupPWL_lambda_random_10.{idx}.csv",
            "validation": f"exps_MixupPWL_lambda_validation_10.{idx}.csv",
            "train": f"exps_MixupPWL_lambda_train_10.{idx}.csv"
        }
